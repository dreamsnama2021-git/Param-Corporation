"use client";

import React from "react";
import Link from "next/link";
import { Phone, Mail, MapPin, ChevronRight } from "lucide-react";
import {
  industries,
  categories,
  priceRanges,
  occasions,
  therapies,
  personalizedGifts,
} from "../app/FooterData";

// Social Icon
const SocialIcon = ({
  href,
  children,
}: {
  href: string;
  children: React.ReactNode;
}) => (
  <a
    href={href}
    target="_blank"
    rel="noopener noreferrer"
    className="w-8 h-8 rounded-full flex items-center justify-center 
    border border-[var(--clr-border-light)] 
    hover:bg-[var(--clr-primary)] transition"
  >
    {children}
  </a>
);

export default function Footer() {
  return (
    <footer className="bg-[var(--clr-bg-dark-deep)] text-gray-300 pt-8 sm:pt-10 md:pt-12 lg:pt-12 xl:pt-16 pb-4 sm:pb-5 md:pb-6">
      <div className="max-w-[1500px] mx-auto px-3 sm:px-4 md:px-6 lg:px-8 xl:px-8">
        {/* ─── MAIN GRID ───────────────── */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 sm:gap-8 md:gap-12 lg:gap-16 xl:gap-14 mb-8 sm:mb-10 md:mb-12">
          {/* ABOUT / LOCATION */}
          <div>
            <img
              src="/logo final param white .png"
              alt="Param Corporation"
              className="h-14 sm:h-16 md:h-18 lg:h-20 xl:h-24 object-contain mb-3 sm:mb-4"
            />

            <p className="text-xs xl:text-sm leading-relaxed mb-3 sm:mb-4 text-gray-400">
              101, KetkiPada Rd, near Timber Green Apartments, Diamond
              Industrial Estate, Dahisar East, Mumbai, Maharashtra 400068
            </p>

            <div className="space-y-2 text-xs xl:text-sm">
              <a
                href="tel:+919820149950"
                className="flex items-center gap-2 hover:text-[var(--clr-primary)] transition-colors"
              >
                <Phone size={12} className="text-[var(--clr-primary)] sm:w-3 sm:h-3 md:w-3.5 md:h-3.5" />
                <span>+91 98201 49950</span>
              </a>

              <a
                href="mailto:info@paramcorporation.com"
                className="flex items-center gap-2 hover:text-[var(--clr-primary)] transition-colors"
              >
                <Mail size={12} className="text-[var(--clr-primary)] sm:w-3 sm:h-3 md:w-3.5 md:h-3.5" />
                <span>info@paramcorporation.com</span>
              </a>

              <div className="flex items-start gap-2">
                <MapPin size={12} className="text-[var(--clr-primary)] mt-0.5 sm:w-3 sm:h-3 md:w-3.5 md:h-3.5" />
                <span className="text-gray-400 text-xs xl:text-sm">
                  Dahisar East, Mumbai, Maharashtra
                </span>
              </div>
            </div>

            {/* SOCIAL */}
            <div className="flex gap-2 sm:gap-2.5 md:gap-3 mt-4 sm:mt-5 md:mt-6">
              {/* Facebook */}
              <SocialIcon href="https://www.facebook.com/parampromotion?mibextid=ZbWKwL">
                <svg viewBox="0 0 24 24" fill="#1877F2" className="w-4 h-4 sm:w-4 sm:h-4 md:w-5 md:h-5">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </SocialIcon>

              {/* Pinterest */}
              <SocialIcon href="https://in.pinterest.com/paramcorporationmumbai/_created/">
                <svg viewBox="0 0 24 24" fill="#E60023" className="w-4 h-4 sm:w-4 sm:h-4 md:w-5 md:h-5">
                  <path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.162-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.143.889 2.741.099.12.112.225.085.345-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.401.165-1.495-.69-2.433-2.878-2.433-4.646 0-3.776 2.748-7.252 7.92-7.252 4.158 0 7.392 2.967 7.392 6.923 0 4.135-2.607 7.462-6.233 7.462-1.214 0-2.354-.629-2.758-1.379l-.749 2.848c-.269 1.045-1.004 2.352-1.498 3.146 1.123.345 2.306.535 3.55.535 6.607 0 11.985-5.365 11.985-11.987C23.97 5.39 18.592.026 11.985.026L12.017 0z" />
                </svg>
              </SocialIcon>

              {/* LinkedIn */}
              <SocialIcon href="https://www.linkedin.com/in/param-corporation-316392184/">
                <svg viewBox="0 0 24 24" fill="#0A66C2" className="w-4 h-4 sm:w-4 sm:h-4 md:w-5 md:h-5">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                </svg>
              </SocialIcon>

              {/* YouTube */}
              <SocialIcon href="https://www.youtube.com/@ParamCorporation-j3f">
                <svg viewBox="0 0 24 24" fill="#FF0000" className="w-4 h-4 sm:w-4 sm:h-4 md:w-5 md:h-5">
                  <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                </svg>
              </SocialIcon>

              {/* Instagram */}
              <SocialIcon href="https://www.instagram.com/paramcorporation_official">
                <svg viewBox="0 0 24 24" className="w-4 h-4 sm:w-4 sm:h-4 md:w-5 md:h-5">
                  <defs>
                    <linearGradient
                      id="insta-grad"
                      x1="0%"
                      y1="100%"
                      x2="100%"
                      y2="0%"
                    >
                      <stop offset="0%" stopColor="#FFDC80" />
                      <stop offset="50%" stopColor="#F56040" />
                      <stop offset="100%" stopColor="#C13584" />
                    </linearGradient>
                  </defs>
                  <path
                    fill="url(#insta-grad)"
                    d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"
                  />
                </svg>
              </SocialIcon>
            </div>
          </div>

          {/* BY CATEGORY */}
          <div>
            <h3
              className="text-white text-sm xl:text-lg font-medium mb-3 sm:mb-3 md:mb-4 pb-1.5 sm:pb-1.5 md:pb-2 
              border-b-2 border-[var(--clr-primary)] inline-block"
            >
              By Category
            </h3>

            <ul className="space-y-1.5 sm:space-y-1.5 md:space-y-2">
              {categories.map((item) => (
                <li key={item.slug}>
                  <Link
                    href={`/categories/all?tab=categories#category-${item.slug}`}
                    className="flex items-center gap-1.5 sm:gap-1.5 md:gap-2 text-xs xl:text-sm 
                    hover:text-[var(--clr-primary)] group transition-all duration-200"
                    scroll={false}
                  >
                    <ChevronRight
                      size={10}
                      className="text-[var(--clr-primary)] group-hover:translate-x-1 transition-transform sm:w-2.5 sm:h-2.5 md:w-3 md:h-3"
                    />
                    <span>{item.name}</span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* BY THERAPY */}
          <div>
            <h3
              className="text-white text-sm xl:text-lg font-medium mb-3 sm:mb-3 md:mb-4 pb-1.5 sm:pb-1.5 md:pb-2 
              border-b-2 border-[var(--clr-primary)] inline-block"
            >
              By Therapy
            </h3>

            <ul className="space-y-1.5 sm:space-y-1.5 md:space-y-2">
              {therapies.map((item) => (
                <li key={item.slug}>
                  <Link
                    href={`/medipride?therapy=${item.slug}`}
                    className="flex items-center gap-1.5 sm:gap-1.5 md:gap-2 text-xs xl:text-sm 
                    hover:text-[var(--clr-primary)] group transition-all duration-200"
                  >
                    <ChevronRight
                      size={10}
                      className="text-[var(--clr-primary)] group-hover:translate-x-1 transition-transform sm:w-2.5 sm:h-2.5 md:w-3 md:h-3"
                    />
                    <span>{item.name}</span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* PERSONALIZED GIFTS */}
          <div>
            <h3
              className="text-white text-sm xl:text-lg font-medium mb-3 sm:mb-3 md:mb-4 pb-1.5 sm:pb-1.5 md:pb-2 
              border-b-2 border-[var(--clr-primary)] inline-block"
            >
              Personalized Gifts
            </h3>

            <ul className="space-y-1.5 sm:space-y-1.5 md:space-y-2">
              {personalizedGifts.map((item) => (
                <li key={item.slug}>
                  <Link
                    href={`/categories/all?tab=personalized#category-${item.slug}`}
                    className="flex items-center gap-1.5 sm:gap-1.5 md:gap-2 text-xs xl:text-sm 
                    hover:text-[var(--clr-primary)] group transition-all duration-200"
                  >
                    <ChevronRight
                      size={10}
                      className="text-[var(--clr-primary)] group-hover:translate-x-1 transition-transform sm:w-2.5 sm:h-2.5 md:w-3 md:h-3"
                    />
                    <span>{item.name}</span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* BY OCCASION */}
          <div>
            <h3
              className="text-white text-sm xl:text-lg font-medium mb-3 sm:mb-3 md:mb-4 pb-1.5 sm:pb-1.5 md:pb-2 
              border-b-2 border-[var(--clr-primary)] inline-block"
            >
              By Occasion
            </h3>

            <ul className="space-y-1.5 sm:space-y-1.5 md:space-y-2">
              {occasions.map((item) => (
                <li key={item.slug}>
                  <Link
                    href={`/categories/all?tab=occasion#category-${item.slug}`}
                    className="flex items-center gap-1.5 sm:gap-1.5 md:gap-2 text-xs xl:text-sm 
                    hover:text-[var(--clr-primary)] group transition-all duration-200"
                  >
                    <ChevronRight
                      size={10}
                      className="text-[var(--clr-primary)] group-hover:translate-x-1 transition-transform sm:w-2.5 sm:h-2.5 md:w-3 md:h-3"
                    />
                    <span>{item.name}</span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>
        </div>

        {/* BOTTOM */}
        <div className="mt-6 sm:mt-8 md:mt-10 pt-3 sm:pt-4 md:pt-5 text-center border-t border-[var(--clr-border-light)]">
          <p className="text-xs  xl:text-sm text-gray-400">
            © 2026 | All rights reserved by Param Corporation
          </p>

          <p className="text-xs  xl:text-sm mt-1.5 sm:mt-2">
            Designed & Developed by{" "}
            <a
              href="https://www.greensmedia.co.in"
              target="_blank"
              rel="noopener noreferrer"
              className="text-[var(--clr-primary)] hover:underline transition"
            >
              Greens Media
            </a>
          </p>
        </div>
      </div>
    </footer>
  );
}